...
class Bubble {
    constructor(canvas) {
        ...
        this.getCanvasSize();//вызывает методы
        this.init();
    }
...
class CanvasBackground {
    ...
    canvasSize() {
        ...
        this.cxt.scale(this.dpr, this.dpr);
    }
    ...
    animate() {
        this.cxt.clearRect(0, 0, this.canvas.clientWidth, this.canvas.clientHeight);
        this.bubblesList.forEach((bubble) => {
            bubble.move();
            this.cxt.translate(bubble.translateX, bubble.translateY);
            this.cxt.beginPath();
            this.cxt.arc(0,0, bubble.size, 0, 2*Math.PI);
            this.cxt.fillStyle = "rgba(" + bubble.color + "," + bubble.alpha + ")";
            this.cxt.fill();
            this.cxt.setTransform(this.dpr, 0, 0, this.dpr, 0, 0);
